...
### 5.2.1 弹窗全局优先级（评审遗留 Y6 收敛 · PM 已拍板）

> **依据**：`config/product-line/码表/style.md` 弹窗体系优先级队列（PM 已确认「导航引导」提到「通知提醒」之前）+ §7.5 勿扰白名单。
...
> **PM 拍板（2026-08-17）**：导航引导（转向/偏航）提到「通知提醒」之上，普通通知不打断导航——转向/偏航是骑行安全信息，社交通知可容忍延迟进列表。style.md 弹窗队列已同步调整。
