...
flowchart TD
    A[收到通知] --> B{来电?}
    B -->|是| C[来电弹窗 最高优先] --> D[关闭/挂断/静音 或 5s 自动关]
    B -->|否| E{弹窗被屏蔽?}
    E -->|是| F[写列表 不弹窗]
    E -->|否| G{正在展示来电?}
    G -->|是| F
...
> - **手机端通知获取**（竞品通用做法，待联网核实后定稿）：iOS 用 **ANCS（Apple Notification Center Service）** 标准 BLE 服务；Android 用 **NotificationListenerService** 通知监听服务。
...
| --- | --- | --- | --- | --- |
| App 消息通知开关 | 支持 | 支持 | PV3.2 支持 | 不支持（benchmark：EDR 整模块删除智联） |
| 普通通知弹窗 | 支持 | 支持 | PV3.2 支持 | 不支持（同上） |
| 消息列表容量 | 20 条 | 5 条 | 20 条（PM 确认） | 不适用 |
| 消息静音/勿扰 | 历史资料支持 | 历史资料支持 | PV3.2 支持 | 不适用 |
