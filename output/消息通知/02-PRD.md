...
- 骑行中收到电话，在码表上识别来电联系人，执行关闭 / 挂断 / 手机端静音。
...
flowchart TD
    A[收到通知] --> B{来电?}
    B -->|是| C[来电弹窗 最高优先] --> D[关闭/挂断/静音 或 通话结束]
    B -->|否| E{弹窗被屏蔽?}
    E -->|是| F[写列表 不弹窗]
    E -->|否| G{正在展示来电?}
    G -->|是| F
...
### 5.2.1 弹窗全局优先级（评审遗留 Y6 收敛 · 判断）

> **依据**：`config/product-line/码表/style.md` 弹窗体系优先级队列 + §7.5 勿扰白名单（勿扰下保留防盗警报/低电量告警/结束骑行确认/转向偏航，不保留智联/普通通知/赛段）。

普通通知在全局弹窗体系中属「通知提醒」档，与智联、赛段提醒同级；来电是通知体系内最高（打断一切普通通知）。完整优先级从高到低：

| 档位 | 弹窗性质 | 与消息通知的关系 |
| --- | --- | --- |
| 🔴 安全预警 | 防盗警报、低电量告警 | 打断普通通知弹窗 |
| 🟠 核心指令 | 结束骑行确认、手动关机 | 打断普通通知弹窗 |
| 🟡 通知提醒 | **来电 > 普通通知 / 智联 / 赛段提醒** | 来电打断普通通知；普通通知与本档其他提醒互斥（后到者等待已展示弹窗结束） |
| 🟢 导航引导 | 转向、偏航 | 普通通知可打断转向/偏航（判断） |
| ⚪ 状态反馈 | 收藏成功、上传成功 | 普通通知可打断 |

> **待 PM 复核**：「通知打断导航引导」一档关系涉及骑行安全（转向/偏航是否应高于普通通知），本条为判断，需 PM 确认。
...
| 未连接码表 | 可编辑本地/账号配置；同步延后至下次连接（保存策略待 App 确认） |
...
| 状态 | 页面表现 | 用户操作 |
| --- | --- | --- |
| 未连接码表 | 顶部提示"连接码表以完成各项设置"，设置置灰 | 连接后重试 |
...
- 支持关闭 / 挂断 / 手机端静音（具体按钮依赖手机系统能力，待确认）。
...
| **勿扰模式**（骑行专注） | 关声音+部分弹窗 | — | 屏蔽声音与部分弹窗，保留安全预警/核心指令/必要导航；**声音按钮静音且不可调节**；被屏蔽的弹窗关闭后不补弹；关闭后恢复开启前声音 |
...
| 勿扰 | 静音（声音按钮不可调） | 不唤醒 | 通知不弹；**白名单保留：防盗警报、低电量告警、结束骑行确认、转向/偏航导航；不保留：智联、普通通知、赛段提醒**（PM 确认） | 写列表 |
...
| call_action | 关闭/挂断/手机端静音 | 系统能力待确认 |
...
| --- | --- | --- | --- | --- |
| App 消息通知开关 | 支持 | 支持 | PV3.2 支持（⚠️ draft 断言，benchmark 无来源，待确认）| 不支持（benchmark：EDR 整模块删除智联） |
| 普通通知弹窗 | 支持 | 支持 | PV3.2 支持（⚠️ draft 断言，benchmark 无来源，待确认）| 不支持（同上） |
| 消息列表容量 | 20 条 | 5 条 | 20 条（PM 确认） | 不适用 |
| 消息静音/勿扰 | 历史资料支持 | 历史资料支持 | PV3.2 支持（⚠️ draft 断言，benchmark 无来源，待确认）| 不适用 |
...
| 8 | C716/C716 Pro 通知能力来源（"PV3.2 支持"为 draft 断言，benchmark §3.5.1 未覆盖，需研发确认起始版本） | 研发/PM |
| 9 | 6 系（C606 等）小屏 + 5 条列表的弹窗布局/截断是否与 C706 同规则 | 设计 |
| 10 | KakaoTalk/Line 适配落地的 App/固件版本号 | PM/发版规划 |
| 8 | BLE 同链路并发：通知突发 vs 配置同步/其他 App 数据共链路的吞吐、背压与优先级 | 固件 |
| 9 | 6 系（C506 等）小屏弹窗布局/截断/来电按钮的降级规则（屏幕规格待确认） | 固件/设计 |
...
| 弹窗全局优先级 | 通知属「通知提醒」档，来电 > 普通通知；安全预警/核心指令打断通知（导航档待 PM 复核） | §5.2.1 |
