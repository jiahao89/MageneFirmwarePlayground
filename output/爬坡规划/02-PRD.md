...
- 触发：到达终点自动浮层。
- 内容：**爬坡名称 + 用时 + 平均坡度 + 总爬升 + 与历史最佳（PR）对比**。
- PR 对比：PR 存码表本地（离线可对比），App 定期同步备份；首次爬坡无 PR 时仅展示本次数据，不显示对比。
...
| PR 历史 | 完成总结对比 | 码表本地存储 + App 同步备份 |
| 断电保护 | 数据完整性 | FIT 实时写入 + 掉电保护（沿用现有 FIT 记录方案）；掉电/异常中断的进行中爬坡开机后标记 abandoned，已完成部分保留 |
...
| GNSS 丢失偏航冻结 | 固件 | 待确认 |
| PR 更新时机（何时判定刷新 PR） | 产品 | 待确认（暂定：成绩优于历史即刷新） |
...
| 版本 | 日期 | 修改人 | 修改内容 |
|---|---|---|---|
| v0.2 | 2026-08-14 | MFP 工作流 | 闭环评审 2 个 🔴：PR 存储定为「码表本地 + App 同步」；补 FIT 断电保护（掉电后进行中爬坡标记 abandoned） |
