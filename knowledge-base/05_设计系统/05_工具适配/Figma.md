...
- 页面按目标系统分区：`顽鹿 App`、`迈金`、`LowDesign`、`GEOID`、`跨端流程`；不要把不同系统的组件混在同一组件页。
- Frame 名称：`[系统] / [端] / [页面] / [状态]`，例如 `迈金 / Device / Sensor List / Default`。
- 组件命名：`[系统]/[类别]/[名称]`，例如 `顽鹿 App/Button/Primary`、`LowDesign/Metric/Hero`、`GEOID/Status/Warning`。
...
- C706、LowDesign 与已确认运行于 C706 平台的 GEOID Frame 固定 `320 × 480px`，先画状态栏和内容高度，再放内容。
- LowDesign 的 Variables 以 `low/` 分组；创建前确认 `low/color/action/volt = #C6FF00`、`low/color/navigation/route = #2D78FF`。不得套用 C706 正式浅色 Token。
