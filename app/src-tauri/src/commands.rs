...
//! 待以下 Issue 填充实现：
//! - Issue #2：工作包状态机与持久化（register / read_work_package / complete）
...
#[tauri::command]
pub async fn save_raw_input(_req: SaveRawInputRequest) -> Result<RawInput, String> {
    Err("save_raw_input: 脚手架占位（Issue #2 工作包持久化实现）".into())
}

#[tauri::command]
pub async fn recognize(_raw_input_id: String) -> Result<RecognitionResult, String> {
    Err("recognize: 脚手架占位（Issue #3 CLI 适配实现）".into())
}

#[tauri::command]
pub async fn register(_raw_input_id: String) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn read_work_package(_request_id: String) -> Result<serde_json::Value, String> {
