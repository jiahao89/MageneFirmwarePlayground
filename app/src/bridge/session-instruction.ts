...
export function buildStartupInstruction(input: StartupInput): string {
  ...
  return [
    ...
    `status 字段合法值词表（写回工作包时只允许使用以下值，禁止自造状态字符串，`,
    `否则 Web 端会判定工作包损坏并进入诊断态）：`,
    `  - "pending_answer"：已写回澄清问题，等待 PM 在 Web 端回答`,
    `  - "pending_review"：已产出待审阅的 PRD，等待 PM 审阅`,
    `  - "processing"：本轮仍在处理中（仅中间态，暂停点不要停留在此值）`,
    ``,
    `PRD 产出后的强制写回（Issue #6 验收 F-4）：${workPackageRelPath} 写入 PRD 文件后，`,
    `必须同步更新工作包 JSON 的以下字段，否则 Web 端无法看到产出：`,
    `  - prdPath：PRD 文件相对 MFP 根目录的路径（必须位于 output/ 下且为 .md 文件，如 output/<需求名>/02-PRD.md）`,
    `  - prdVersion：在上一版本基础上 +1（首次为 1，必须是 ≥1 的整数）`,
    `  - artifacts：追加本次产出的文件路径`,
    `  - status：置为 "pending_review"（待 PM 审阅）`,
    `  - taskCard.currentPhase：推进到下一阶段`,
    `  注意：PRD 文件必须真实写入且非空；只登记路径不写文件会被产物门禁退回处理中。`,
    ``,
    `禁止事项：`,
